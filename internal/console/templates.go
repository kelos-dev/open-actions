...
<head>
  ...
  <style>
    :root{color-scheme:dark;font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;background:#0d1117;color:#f0f6fc}*{box-sizing:border-box}body{margin:0;background:#0d1117;color:#f0f6fc;font-size:14px}a{color:#58a6ff;text-decoration:none}a:hover{text-decoration:underline}.topbar{height:64px;display:flex;align-items:center;padding:0 24px;border-bottom:1px solid #21262d;background:#010409}.brand{display:flex;align-items:center;gap:10px;color:#f0f6fc;font-weight:600}.brand:hover{text-decoration:none}.brand-mark{display:grid;place-items:center;width:30px;height:30px;border:1px solid #30363d;border-radius:7px;color:#58a6ff;font-size:12px;font-weight:800}.page{width:min(1180px,100%);margin:0 auto;padding:28px 32px 56px}.breadcrumbs{display:flex;align-items:center;gap:8px;margin-bottom:22px;color:#8b949e;font-size:14px}.breadcrumbs strong{color:#f0f6fc}.run-heading{display:flex;align-items:flex-start;justify-content:space-between;gap:24px;margin-bottom:22px}.run-heading h1{margin:0 0 8px;font-size:24px;font-weight:600;line-height:1.25;letter-spacing:-.01em}.muted{color:#8b949e}.workflow-path{overflow-wrap:anywhere}.summary{display:grid;grid-template-columns:auto 1fr;gap:14px;padding:20px;border:1px solid #30363d;border-radius:6px;background:#0d1117}.status-mark{display:grid;place-items:center;flex:0 0 auto;width:24px;height:24px;border:2px solid currentColor;border-radius:50%;font-size:14px;font-weight:800}.status-mark.succeeded{color:#3fb950}.status-mark.failed{color:#f85149}.status-mark.running{color:#d29922;border-style:dashed}.status-mark.queued{color:#8b949e}.status-mark.waiting-for-approval{color:#a371f7}.status-mark.succeeded::after{content:"✓"}.status-mark.failed::after{content:"×"}.status-mark.running::after{content:""}.status-mark.queued::after{content:"·"}.status-mark.waiting-for-approval::after{content:"!"}.summary-title{margin:1px 0 5px;font-size:16px;font-weight:600}.summary-meta{display:flex;flex-wrap:wrap;gap:6px 14px;color:#8b949e;font-size:13px}.summary-meta code{padding:2px 5px;border:1px solid #30363d;border-radius:5px;background:#161b22;color:#c9d1d9;font:12px ui-monospace,SFMono-Regular,Consolas,monospace}.section-title{display:flex;align-items:center;gap:8px;margin:30px 0 12px;font-size:16px;font-weight:600}.count{min-width:20px;padding:1px 6px;border-radius:20px;background:#30363d;color:#c9d1d9;font-size:12px;text-align:center}.jobs{overflow:hidden;border:1px solid #30363d;border-radius:6px}.job{display:grid;grid-template-columns:minmax(220px,1fr) 150px 160px 180px 150px;align-items:center;min-height:68px;padding:12px 16px;border-bottom:1px solid #21262d}.job:last-child{border-bottom:0}.job:hover{background:#161b2266}.job-link{display:flex;align-items:center;gap:12px;min-width:0;color:#f0f6fc;font-weight:600}.job-link:hover{color:#58a6ff;text-decoration:none}.job-status{width:18px;height:18px;font-size:11px}.job-name{min-width:0}.job-name span{display:block;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.job-name small{display:block;margin-top:3px;color:#8b949e;font-weight:400}.cell{color:#8b949e;font-size:13px}.status-text{color:#c9d1d9}.empty{padding:32px;text-align:center;color:#8b949e}@media(max-width:760px){.topbar{padding:0 16px}.page{padding:22px 16px 40px}.run-heading{display:block}.summary{padding:16px}.job{grid-template-columns:1fr auto;gap:8px}.job .environment,.job .runner,.job .started{display:none}.job .status-text{text-align:right}}
  </style>
</head>
<body>
  <nav class="topbar" aria-label="Global"><a class="brand" href="/"><span class="brand-mark" aria-hidden="true">OA</span><span>Open Actions</span></a></nav>
  <main class="page">
    ...
    <section class="jobs" aria-label="Jobs">
      {{range .Jobs}}<div class="job">
        ...
        <span class="cell environment">{{if .Environment}}{{.Environment}}{{else}}—{{end}}</span><span class="cell status-text">{{.Status}}</span><span class="cell runner">{{if .Runner}}{{.Runner}}{{else}}—{{end}}</span><span class="cell started">{{if .Duration}}{{.Duration}}{{else if .Started}}In progress{{else}}Not started{{end}}</span>
    ...
    <div class="layout">
      ...
      <section>
        <header class="job-header">
          <div><h1>{{.JobName}}</h1><div class="job-meta"><span>{{.Status}}</span>{{if .Environment}}<span>Environment: {{.Environment}}</span>{{end}}<span>{{.Runner}}</span>{{if .Duration}}<span>{{.Duration}}</span>{{end}}<code class="optional">{{.ShortRevision}}</code></div></div>
