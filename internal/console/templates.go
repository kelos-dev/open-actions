  ...
  <main class="page">
    ...
    <section class="summary" aria-label="Workflow run summary">
      ...
      <div>
        ...
        <div class="summary-meta"><span>{{.RefName}}</span><code title="{{.Revision}}">{{.ShortRevision}}</code>{{if .Started}}<time data-time="{{.Started}}">{{.Started}}</time>{{end}}<span data-duration="{{.RunURL}}" title="Workflow run duration">{{.Duration}}</span></div>
      </div>
    </section>
  ...
  <main class="page">
    ...
    <div class="layout">
      <aside class="sidebar" aria-label="Workflow jobs">
        ...
        {{range .Jobs}}<a class="side-job{{if .Selected}} selected{{end}}" href="{{.URL}}"><span class="side-dot {{.StatusClass}}" aria-hidden="true"></span><span class="side-name">{{.DisplayName}}</span><span class="side-duration" data-duration="{{.Name}}" title="Job duration">{{.Duration}}</span></a>{{end}}
      </aside>
      <section>
        <header class="job-header">
          <div><h1>{{.JobName}}</h1><div class="job-meta"><span>{{.Status}}</span><span>{{.Runner}}</span><span data-duration="{{.JobResourceName}}" title="Job duration">{{.Duration}}</span><code class="optional">{{.ShortRevision}}</code></div></div>
