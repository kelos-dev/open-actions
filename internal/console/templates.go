...
<body data-stream-url="{{.StreamURL}}">
  ...
  <script>
    ...
    function estimateClusterHeight(cluster){
      ...
      for(const record of cluster.records){const entry=record.entry;let text=entry.text||'';if(entry.kind==='input'||entry.kind==='step-output')text=(entry.kind==='input'?'with ':'output ')+text;let rows=0;for(const part of text.split('\n'))rows+=Math.max(1,Math.ceil(Math.max(1,part.length)/width));if(entry.properties&&(entry.properties.title||entry.properties.file||entry.properties.line))rows++;height+=Math.max(20,rows*19.8)}
      return height;
    }
    ...
    function addEntry(entry,appender,renderNewClusters,groupsToCollapse){
      ...
      const record={entry,number:++lineNumber};cluster.records.push(record);if(cluster.rendered)appender.append(cluster.element,createLine(record));else updateClusterEstimate(cluster);
    }
